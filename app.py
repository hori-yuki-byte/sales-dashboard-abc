...
# 商談ログ②の固定接続先（Streamlit Secrets で管理）
DEFAULT_SHEET_URL = st.secrets.get("DEFAULT_SHEET_URL", "")
...
def col_uu(df, filter_col, pattern, id_col="顧客ID", regex=False, exclude_col=None, exclude_patterns=None) -> int:
    ...
    if filter_col not in df.columns:
        return 0
    pat = _get_regex(pattern) if regex else pattern
    mask = get_col(df, filter_col).str.contains(pat, na=False, regex=regex)
    if exclude_col and exclude_patterns and exclude_col in df.columns:
        for ep in exclude_patterns:
            mask &= ~get_col(df, exclude_col).str.contains(_get_regex(ep), na=False, regex=True)
    matched = df[mask]
...
def render_alerts(kpi: dict, ganchi: dict, chakuza: dict, df_src: pd.DataFrame, thresholds: dict, per_person_df: pd.DataFrame = None):
    ...
    def _person_row_from_df(row):
        ...
        pcz_pre = {
            "プレ着座率":   pk.get("プレ着座率", "-"),
            "再プレ着座率": pk.get("再プレ着座率", "-"),
            "プレ成約率":   pk.get("プレ成約率", "-"),
            "プレ予定UU":   pk.get("プレ予定UU", 0),
            "再プレ予定UU": pk.get("再プレ予定UU", 0),
        }
...
""", unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        if st.session_state.get("_password_ok"):
            if st.button("🔓 ログアウト", key="pw_logout"):
                _get_cookie_controller().remove("hatch_dashboard_pw")
                st.session_state.pop("_password_ok", None)
                st.rerun()
            st.divider()
...
公開後は「取得・更新」ボタンを押すだけで最新データに更新されます。
    ...
    def render_kpi_section(date_col: str, section_key: str, title: str, default_period: int):
        ...
        r1[2].metric("ブリッジUU", kpi["ブリッジUU"],  help="報告種別＝アポ かつ 結果にプレ日程確定を含む")
        r1[3].metric("ブリッジ率", kpi["ブリッジ率"],  help="ブリッジUU ÷ アポUU")

        # ── ② プレ ──────────────────────────────────────
        st.caption("② プレ")
        r2 = st.columns(6)
        r2[0].metric("プレUU",       kpi["プレUU"],       help="報告種別＝プレ（リスケ・日程確定除く）")
        r2[1].metric("プレリスケUU", kpi["プレリスケUU"], help="報告種別＝プレ かつ 結果にリスケを含む")
        r2[2].metric("プレ飛びUU",   kpi["プレ飛びUU"],   help="報告種別にプレ飛びを含む")
        r2[3].metric("プレ言質UU",   g["プレ言質UU"],     help="最後がプレ→契約or次回契約予定")

        # ── ③ 再プレ ────────────────────────────────────
        st.caption("③ 再プレ")
        r3 = st.columns(6)
        r3[0].metric("再プレUU",       kpi["再プレUU"],       help="報告種別＝再プレ（リスケ・日程確定除く）")
        r3[1].metric("再プレリスケUU", kpi["再プレリスケUU"], help="報告種別＝再プレ かつ 結果にリスケを含む")
        r3[2].metric("再プレ飛びUU",   kpi["再プレ飛びUU"],   help="報告種別に再プレ飛びを含む")
        r3[3].metric("再プレ言質UU",   g["再プレ言質UU"],     help="最後が再プレ→契約or次回契約予定")

        # ── ④ 契約 ──────────────────────────────────────
        st.caption("④ 契約")
        r4 = st.columns(6)
        r4[0].metric("次回契約予定UU", kpi["次回契約予定UU"], help="結果に次回契約予定を含む")
        r4[1].metric("契約UU",         kpi["契約UU"],         help="結果＝契約（完全一致）")
        r4[2].metric("契約リスケUU",   kpi["契約リスケUU"],   help="報告種別に契約を含む かつ 結果にリスケを含む")
        r4[3].metric("契約飛びUU",     kpi["契約飛びUU"],     help="契約予定飛び or 契約予定調整＋失注")
        r4[4].metric("失注UU",         kpi["失注UU"],         help="報告種別または結果＝失注")

        # ── 率 ──────────────────────────────────────────
        st.caption("📈 率")
        r5 = st.columns(6)
        r5[0].metric("成約率",               kpi["成約率"],                         help="契約÷（契約＋失注＋プレ飛び＋再プレ飛び＋契約飛び）")
    ...
    # =============================================
        ...
        if df_raw.empty or "次回アクション日" not in df_raw.columns:
            ...
            _riske = _ketsu.str.contains("リスケ日程確定", na=False)
            _conditions = [
            ...
            _choices = ["プレ予定", "再プレ予定", "契約予定", "再プレ予定", "プレ予定"]
            df_sch["予定種別"] = np.select(_conditions, _choices, default="")
            df_sch = df_sch[df_sch["予定種別"] != ""]
        ...
        if df_raw.empty or "報告種別" not in df_raw.columns:
            ...
            if pre_riske_df.empty and re_pre_riske_df.empty:
                ...
                if counts_df.empty:
                    ...
                    # 空き日数の計算（ベクトル化: O(n) groupby事前計算）
                    # プレリスケ → アポの営業日 〜 次回予定日
                    # 再プレリスケ → プレの営業日 〜 次回予定日
                    _pre_mask = get_col(df_raw, "報告種別").str.contains(PRE_PATTERN, na=False, regex=True)
                    _apo_mask = get_col(df_raw, "報告種別").str.contains(APO_PATTERN, na=False, regex=True)
                    _pre_dates = df_raw[_pre_mask].groupby("顧客ID")["営業日"].max()
                    _apo_dates = df_raw[_apo_mask].groupby("顧客ID")["営業日"].max()
                    _is_re_pre = latest_riske["最新リスケ種別"].astype(str).str.contains(
                        RE_PRE_PATTERN, na=False, regex=True
                    )
                    latest_riske["基準日"] = np.where(
                        _is_re_pre,
                        latest_riske["顧客ID"].map(_pre_dates),
                        latest_riske["顧客ID"].map(_apo_dates),
                    )
...
_APO_SLOT_DURATION  = 90
_APO_SLOT_INCREMENT = 30
_APO_DAY_START      = 8
_APO_LAST_START     = 22
...
_CATEGORY_RULES = [
    ...
    ("プレ枠",        ["プレ枠", "プレ入り"]),
    ("ロープレ・相席", ["ロープレ", "相席"]),
    ("アポ以外の営業", ["【ウェブフリ】", "プレ", "契約", "再プレ", "再アポ", "ボイチェン", "契約予定"]),
    ("移動",          ["移動"]),
    ("施策",          ["ブラック", "エメグリ", "イエロー", "ルビレ", "ホワイト", "ジャイキリ", "プレ対策", "研修", "1on1", "MT"]),
    ("事務作業",      ["タスク", "商談ログ", "日報", "アウトプット", "報告", "Cデータ", "リマインド", "タイトルなし"]),
]

_APO_EVENT_TEMPLATES = [
    ("【ウェブフリ】無料カウンセリング",        True,  90),
    ("【ウェブフリ】無料カウンセリング 鈴木様",  True,  90),
    ("プレ枠",                               True,  90),
    ("プレ枠",                               True,  60),
    ("プレ枠",                               True, 120),
    ("【ウェブフリ】プレ 田中様",              True,  90),
    ("【ウェブフリ】再プレ 山田様",            True,  90),
    ("【ウェブフリ】ボイチェン 佐藤様",        True,  90),
    ("【ウェブフリ】契約予定 伊藤様",          True,  60),
    ("【ウェブフリ】資金繰り 渡辺様",          True,  60),
    ("ロープレ",                              True,  60),
    ("相席 田中マネージャー",                  True,  60),
    ("移動",                                  True,  30),
    ("移動（渋谷→新宿）",                    True,  60),
    ("zoom不可（自分の時間）",                True,  60),
    ("ご飯",                                  True,  60),
    ("マネージャーMT",                        True,  60),
    ("ログ・日報",                            False, 30),
    ("日報",                                  False, 30),
...
_APO_DEMO_MEMBERS = [
    {"name": "田中裕喜",   "email": "tanaka@example.com",    "team": "営業A"},
    {"name": "山田和弥",   "email": "yamada@example.com",    "team": "営業A"},
    {"name": "鈴木大和",   "email": "suzuki@example.com",    "team": "営業A"},
    {"name": "伊藤颯太",   "email": "ito@example.com",       "team": "営業A"},
    {"name": "渡辺健斗",   "email": "watanabe@example.com",  "team": "営業A"},
    {"name": "佐藤奈々恵", "email": "sato@example.com",      "team": "営業B"},
    {"name": "倉橋牧己",   "email": "kurahashi@example.com", "team": "営業B"},
    {"name": "瀧本麻代",   "email": "takimoto@example.com",  "team": "営業B"},
    {"name": "中村春菜",   "email": "nakamura@example.com",  "team": "営業B"},
    {"name": "小林美咲",   "email": "kobayashi@example.com", "team": "営業B"},
    {"name": "荻野真帆",   "email": "ogino@example.com",     "team": "営業C"},
    {"name": "高原若菜",   "email": "takahara@example.com",  "team": "営業C"},
    {"name": "遠藤里奈",   "email": "endo@example.com",      "team": "営業C"},
    {"name": "松本陽子",   "email": "matsumoto@example.com", "team": "営業C"},
    {"name": "加藤誠",     "email": "kato@example.com",      "team": "営業C"},
]

def _apo_get_slots(date):
    slots = []
    t    = _dt.datetime.combine(date, _dt.time(_APO_DAY_START, 0))
    last = _dt.datetime.combine(date, _dt.time(_APO_LAST_START, 0))
    while t <= last:
        slots.append((t, t + _dt.timedelta(minutes=_APO_SLOT_DURATION)))
        t += _dt.timedelta(minutes=_APO_SLOT_INCREMENT)
    return slots

def _apo_classify(title, busy):
    if "プレ枠" in title:
        return "pre"
    if "アポ" in title or busy:
        return "busy"
    return "free"

def _apo_categorize(title):
    if not title.strip():
        return "事務作業"
...
def _apo_calculate(events, date):
    slots  = _apo_get_slots(date)
    parsed = [(ev["start"], ev["end"], _apo_classify(ev["title"], ev["busy"])) for ev in events]
    result = {"available": [], "pre": [], "busy": []}
    for s, e in slots:
        status = "available"
        for es, ee, cat in parsed:
            if es < e and ee > s:
                if cat == "busy":
                    status = "busy"
                    break
                elif cat == "pre" and status == "available":
                    status = "pre"
        result[status].append((s, e))
    return result

def _apo_summarize(calc):
    a, p, b = len(calc["available"]), len(calc["pre"]), len(calc["busy"])
    return {
        "available_slots": a, "available_hours": round(a * _APO_SLOT_DURATION / 60, 1),
        "pre_slots":       p, "pre_hours":       round(p * _APO_SLOT_DURATION / 60, 1),
        "busy_slots":      b,
    }

def _apo_breakdown(events):
    cats = {cat: {"count": 0, "minutes": 0.0} for cat, _ in _CATEGORY_RULES}
    cats["その他"] = {"count": 0, "minutes": 0.0}

    busy = sorted([ev for ev in events if ev["busy"]], key=lambda x: x["start"])
    if not busy:
        return {k: f'{v["count"]}件 / {v["minutes"]/60:.1f}h' for k, v in cats.items()}

    # 重なっている予定をグループ化
    groups, grp, grp_end = [], [busy[0]], busy[0]["end"]
    for ev in busy[1:]:
        if ev["start"] < grp_end:
            grp.append(ev)
            grp_end = max(grp_end, ev["end"])
        else:
            groups.append(grp)
            grp, grp_end = [ev], ev["end"]
    groups.append(grp)

    def _priority(ev):
        cat = _apo_categorize(ev["title"])
        return _CATEGORY_PRIORITY.index(cat) if cat in _CATEGORY_PRIORITY else len(_CATEGORY_PRIORITY)

    for grp in groups:
        winner = min(grp, key=_priority)
        cat = _apo_categorize(winner["title"])
        cats[cat]["count"] += 1
        cats[cat]["minutes"] += (winner["end"] - winner["start"]).seconds / 60

    return {k: f'{v["count"]}件 / {v["minutes"]/60:.1f}h' for k, v in cats.items()}

def _apo_gen_demo(date, seed=0):
    import random
    rng    = random.Random(seed)
    events = []
    cur    = _dt.datetime.combine(date, _dt.time(_APO_DAY_START, 0))
    limit  = _dt.datetime.combine(date, _dt.time(22, 0))
    while cur < limit:
        if rng.random() < 0.55:
            title, busy, dur = rng.choice(_APO_EVENT_TEMPLATES)
            end = cur + _dt.timedelta(minutes=dur)
            if end <= _dt.datetime.combine(date, _dt.time(23, 30)):
                events.append({"title": title, "start": cur, "end": end, "busy": busy})
            cur = end
        else:
            cur += _dt.timedelta(minutes=30)
    return events

_APO_SCOPES          = ["https://www.googleapis.com/auth/calendar.readonly"]
_APO_MEMBERS_FILE    = Path(__file__).parent / "apo_members.json"
_USER_AUTH_SCOPES    = ["openid", "email", "profile"]
_USER_AUTH_STATE     = "user_login"
_CALENDAR_AUTH_STATE = "calendar_auth"

# 内訳カテゴリの優先順位（重なった予定がある場合、上位カテゴリのみカウント）
_CATEGORY_PRIORITY = [
    "アポ",           # 1位
    "アポ以外の営業", # 2位
    "ロープレ・相席", # 3位
...
def _apo_load_members():
    if _APO_MEMBERS_FILE.exists():
        return json.loads(_APO_MEMBERS_FILE.read_text(encoding="utf-8"))
    return _APO_DEMO_MEMBERS


def _apo_get_oauth_config():
    """Streamlit SecretsからクライアントシークレットとリダイレクトURIを取得"""
    cfg = st.secrets.get("google_oauth", {})
    raw = cfg.get("client_secrets", "")
    if not raw:
        return None, None
    return json.loads(raw), cfg.get("redirect_uri", "http://localhost:8501")
...
def _apo_load_creds():
    ...
    # セッションに有効なトークンがあればそれを使う
    data = st.session_state.get("_apo_creds_json")
    if data:
        creds = Credentials.from_authorized_user_info(json.loads(data), _APO_SCOPES)
        if creds.valid:
            return creds
        if creds.expired and creds.refresh_token:
            try:
                creds.refresh(Request())
                st.session_state["_apo_creds_json"] = creds.to_json()
                return creds
            except Exception:
                pass

    # Secrets の refresh_token で自動認証
    cfg = st.secrets.get("google_oauth", {})
    refresh_token = cfg.get("refresh_token", "")
    if not refresh_token:
        return None
    client_secrets = json.loads(cfg.get("client_secrets", "{}"))
    web = client_secrets.get("web", {})
    creds = Credentials(
        token=None,
...
def _apo_save_creds(creds):
    st.session_state["_apo_creds_json"] = creds.to_json()
    # refresh_token を session_state に保持（初回認証後の表示用）
    token_data = json.loads(creds.to_json())
    if token_data.get("refresh_token"):
        st.session_state["_apo_new_refresh_token"] = token_data["refresh_token"]


def _apo_make_auth_url():
    import urllib.parse
    client_secrets, redirect_uri = _apo_get_oauth_config()
    ...
    params = {
        ...
        "response_type": "code",
        "scope":         " ".join(_APO_SCOPES),
        "access_type":   "offline",
        "prompt":        "consent",
        "state":         _CALENDAR_AUTH_STATE,
    }
    return web.get("auth_uri", "https://accounts.google.com/o/oauth2/auth") + "?" + urllib.parse.urlencode(params)


def _apo_exchange_code(code):
    ...
    _apo_save_creds(creds)
    return True


def _apo_fetch_api_oauth(creds, user_email, date):
    try:
    ...
    tz    = "+09:00"
    t_min = _dt.datetime.combine(date, _dt.time(0, 0)).isoformat() + tz
    t_max = _dt.datetime.combine(date, _dt.time(23, 59, 59)).isoformat() + tz
    try:
        raw = svc.events().list(
            calendarId=user_email, timeMin=t_min, timeMax=t_max,
    ...
    for item in raw.get("items", []):
        ...
        if "dateTime" not in s:
            continue
        ev_start = _dt.datetime.fromisoformat(s["dateTime"]).replace(tzinfo=None)
        ev_end   = _dt.datetime.fromisoformat(e["dateTime"]).replace(tzinfo=None)
        title    = item.get("summary", "")
        busy     = item.get("transparency", "opaque") != "transparent"
        events.append({"title": title, "start": ev_start, "end": ev_end, "busy": busy})
    return events
...
def _is_email_allowed(email: str) -> bool:
    ...
    if email in cfg.get("allowed_emails", []):
        return True
    domain = cfg.get("allowed_domain", "")
    return bool(domain and email.endswith(f"@{domain}"))


def _make_user_login_url():
    import urllib.parse
    client_secrets, redirect_uri = _apo_get_oauth_config()
    if not client_secrets:
        return None
    ...
    params = {
        ...
        "redirect_uri":  redirect_uri,
        "response_type": "code",
        "scope":         " ".join(_USER_AUTH_SCOPES),
        "access_type":   "offline",
        "prompt":        "select_account",
        "state":         _USER_AUTH_STATE,
    }
    return web.get("auth_uri", "https://accounts.google.com/o/oauth2/auth") + "?" + urllib.parse.urlencode(params)
...
def _exchange_code_for_email(code: str) -> str:
    ...
    with urllib.request.urlopen(req2) as resp2:
        return json.loads(resp2.read()).get("email", "")
...
def _check_password():
    ...
    if not correct:
        return  # パスワード未設定なら素通り

    # cookieに保存された認証印
    cookie = _get_cookie_controller().get("hatch_dashboard_pw")
    if cookie == correct:
    ...
    with mid:
        ...
        if st.button("ログイン", use_container_width=True, key="_pw_submit"):
            if pw == correct:
                st.session_state["_password_ok"] = True
                _get_cookie_controller().set("hatch_dashboard_pw", pw, max_age=2_592_000)  # 30日
                st.rerun()
            else:
                st.error("パスワードが違います")
    st.stop()
...
def _check_login():
    if st.session_state.get("_user_email"):
        return

    # OAuthコールバック（Googleからのリダイレクト）
    params = st.query_params
    if "code" in params and params.get("state") == _USER_AUTH_STATE:
        try:
            email = _exchange_code_for_email(params["code"])
            st.query_params.clear()
            if _is_email_allowed(email):
                st.session_state["_user_email"] = email
                _get_cookie_controller().set("hatch_user_email", email, max_age=2_592_000)  # 30日
                st.rerun()
            else:
                st.error(f"⛔ このアカウント（{email}）はアクセス権限がありません。")
                login_url = _make_user_login_url()
                if login_url:
                    st.link_button("別のアカウントでログイン", login_url)
                st.stop()
        except Exception as e:
            st.query_params.clear()
            st.error(f"認証エラー: {e}")
    ...
    # cookieから復元（ブラウザ再訪問・リロード時）
    stored_email = _get_cookie_controller().get("hatch_user_email")
    if stored_email and _is_email_allowed(stored_email):
        st.session_state["_user_email"] = stored_email
        return

    client_secrets, _ = _apo_get_oauth_config()
    if not client_secrets:
        st.error("Streamlit Secrets に google_oauth が設定されていません")
        st.stop()

    _, mid, _ = st.columns([1, 2, 1])
    with mid:
        st.markdown("## 🔐 ログイン")
        st.markdown("ハッチのGoogleアカウント（@hatch-inc.info）でログインしてください。")
        login_url = _make_user_login_url()
        if login_url:
...
def _render_apo_page():
    st.markdown("## 📅 アポ枠ダッシュボード")

    selected_date = st.date_input("日付", value=_date.today(), key="apo_date")

    creds   = None
    members = _apo_load_members()

    if True:
        if "code" in st.query_params and st.query_params.get("state") == _CALENDAR_AUTH_STATE:
            try:
                if _apo_exchange_code(st.query_params["code"]):
                    st.query_params.clear()
                    st.rerun()
            except Exception as e:
                st.error(f"認証エラー: {e}")
        ...
        if creds is None:
            ...
            if client_secrets is None:
                st.error("Streamlit Secrets に google_oauth が設定されていません")
                return
            auth_url = _apo_make_auth_url()
            st.info("共有Googleアカウントで認証してください")
            st.link_button("🔑 Googleにログインして認証する", auth_url)
            return

        col_status, col_logout = st.columns([4, 1])
        col_status.success("✅ Google認証済み")
        if col_logout.button("ログアウト", key="apo_logout"):
            st.session_state.pop("_apo_creds_json", None)
            st.session_state.pop("_apo_new_refresh_token", None)
            st.rerun()

        new_rt = st.session_state.get("_apo_new_refresh_token")
        cfg = st.secrets.get("google_oauth", {})
        if new_rt and not cfg.get("refresh_token"):
            with st.expander("🔑 次回から自動ログインにする（1回だけ設定）", expanded=True):
                st.info("以下の refresh_token を Streamlit Cloud の Secrets に追加すると、次回から自動ログインになります。")
                st.code(f'refresh_token = "{new_rt}"', language="toml")
                st.caption("Streamlit Cloud → アプリSettings → Secrets → [google_oauth] セクションに追加 → Save changes")

    teams  = sorted({m["team"] for m in members if m.get("team", "")})
    names  = [m["name"] for m in members]
    filter_label = st.selectbox("表示対象", ["全員"] + teams + names, key="apo_filter")

    if filter_label == "全員":
        target = members
    elif filter_label in teams:
        target = [m for m in members if m["team"] == filter_label]
    else:
        target = [m for m in members if m["name"] == filter_label]

    rows = []
    with st.spinner("カレンダー集計中..."):
        for member in target:
            events = _apo_fetch_api_oauth(creds, member["email"], selected_date)
            calc = _apo_calculate(events, selected_date)
            s    = _apo_summarize(calc)
            bd   = _apo_breakdown(events)
            rows.append({
                "チーム":           member.get("team", ""),
                "名前":             member["name"],
                "アポ可能枠":       s["available_slots"],
                "埋まり枠数":       s["busy_slots"],
                "┃アポ":           bd["アポ"],
                "┃アポ以外の営業":  bd["アポ以外の営業"],
                "┃プレ枠":         bd["プレ枠"],
    ...
    total_avail = int(df["アポ可能枠"].sum())

    def _parse_count(s):
        try: return int(str(s).split("件")[0])
        except: return 0

    total_apo = sum(_parse_count(v) for v in df["┃アポ"])
    total_pre = sum(_parse_count(v) for v in df["┃プレ枠"])

    c1, c2, c3, c4 = st.columns(4)
    c1.metric("対象メンバー",     f"{len(df)}名")
    c2.metric("アポ可能枠（合計）", f"{total_avail}枠")
    c3.metric("アポ（合計）",     f"{total_apo}件")
    c4.metric("プレ枠（合計）",   f"{total_pre}件")

    st.divider()
    st.subheader("メンバー別 アポ枠状況")
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "アポ可能枠":      st.column_config.NumberColumn("アポ可能枠",     help="何も予定のない90分枠の数"),
            "埋まり枠数":      st.column_config.NumberColumn("埋まり枠数",     help="busyな予定が重なっている90分枠の数"),
            "┃アポ":          st.column_config.TextColumn("┃アポ"),
            "┃アポ以外の営業": st.column_config.TextColumn("┃アポ以外営業"),
            "┃プレ枠":        st.column_config.TextColumn("┃プレ枠"),
            "┃ロープレ・相席": st.column_config.TextColumn("┃ロープレ相席"),
            "┃移動":          st.column_config.TextColumn("┃移動"),
            "┃施策":          st.column_config.TextColumn("┃施策"),
            "┃事務作業":      st.column_config.TextColumn("┃事務作業"),
            "┃その他":        st.column_config.TextColumn("┃その他"),
        },
    )
    st.caption("📌 **定義：** アポ可能枠＝何も予定のない90分枠 ／ 埋まり枠＝busyな予定が重なっている90分枠 ／ 1日の最大枠数＝29枠（8:00〜22:00、90分・30分刻み）")

    st.divider()
    st.subheader("枠数分布")
    fig = go.Figure()
    fig.add_trace(go.Bar(name="アポ可能枠", x=df["名前"], y=df["アポ可能枠"], marker_color="#4CAF50"))
    fig.add_trace(go.Bar(name="埋まり枠",   x=df["名前"], y=df["埋まり枠数"], marker_color="#EF5350"))
    fig.update_layout(
